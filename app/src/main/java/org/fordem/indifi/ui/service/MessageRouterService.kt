...
//                                    }
    ...
    private suspend fun receiveHelloMessage(obj: JSONObject, socket: Socket) {
        ...
        for (i in 0 until devicesArray.length()) {
            ...
            // TODO:  : This duplicate check seems to be failing, since multiples of the same device are being inserted
...
//
    ...
    fun broadcastMessageToAllGMs(dataToSend: String) {
        CoroutineScope(Dispatchers.IO).launch {
            ...
            // Send Broadcast to all GMs except self if GO
            allDevices.forEach { device ->
                if (!device.isGroupOwner) {
                    try {
                        ...
                        socket.close()
                        Handler(Looper.getMainLooper()).post {
                            Log.d("BROADCAST", "Sent to ${device.ip}, message: $dataToSend")
                        }
                    } catch (e: Exception) {
                        Handler(Looper.getMainLooper()).post {
