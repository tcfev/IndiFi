...
class MulticastService : Service() {
    ...
    inner class LocalBinder : Binder() {
        fun getService(): MulticastService = this@MulticastService
    }
    ...
    private fun startListeningForMulticastMessages() {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                // Pick correct interface for LC/GO rules
                val localIp = if (!myDevice?.isRelayDevice!!) {
                    getMulticastCapableIp() // LC-GO → wlan0
                } else {
                    getMulticastGMIp() // LC-GM → p2p0
                }
                val iface = NetworkInterface.getByInetAddress(localIp)
...
//                val targetIP = when {
////                    targetIP.startsWith("192.168.") -> getWlan0Ip() // Legacy/Hotspot (LC)
