...
//        startListeningForMulticastMessages()
    ...
    private val multicastGroup: String = "230.0.0.1"
    ...
    private fun startListeningForMulticastMessages() {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                val group = InetAddress.getByName(multicastGroup/*"230.0.0.1"*/)
...
//                val localIp = getLocalIp()
    ...
    private fun getCorrectNetworkInterface(): NetworkInterface {
//        val localIp = getLocalIp()
//        val localIp = getP2p0Ip()
        val localIp = getWlan0Ip()
        return NetworkInterface.getByInetAddress(localIp)
            ?: throw IllegalStateException("Unable to find interface for IP $localIp")
    }
    ...
    private fun getLocalIp(): InetAddress {
        ...
        throw IllegalStateException("No suitable local IP found (are you connected or hosting a hotspot?)")
    }

    private fun getP2p0Ip(): InetAddress {
        val interfaces = NetworkInterface.getNetworkInterfaces()
        for (iface in interfaces) {
            if (iface.name == "p2p0" && iface.isUp) {
        ...
        throw IllegalStateException("No IP on p2p0 — are you a GO?")
    }

    private fun getWlan0Ip(): InetAddress {
        val interfaces = NetworkInterface.getNetworkInterfaces()
        for (iface in interfaces) {
            if ((iface.name == "ap0" || iface.name == "wlan0") && iface.isUp) {
                val addrs = iface.inetAddresses
                while (addrs.hasMoreElements()) {
                    val addr = addrs.nextElement()
                    if (addr is Inet4Address && !addr.isLoopbackAddress) {
...
//        }
    ...
    fun startChatServer(
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                serverSocket = ServerSocket(UNICAST_PORT)
                Log.d("TCP", "Server started on port $UNICAST_PORT")
...
//        CoroutineScope(Dispatchers.IO).launch {
//            try {
//                val netInterface = NetworkInterface.getByName("p2p0")
//                val localIp = netInterface?.inetAddresses?.toList()
//                    ?.firstOrNull { !it.isLoopbackAddress && it is Inet4Address } as? Inet4Address
//
//                if (localIp == null) {
//                    Log.e("TCP", "No IP found for interface p2p0")
//                    return@launch
//                }
//
//                val bindAddress = InetSocketAddress(localIp, UNICAST_PORT)
//                val serverSocket = ServerSocket()
//                serverSocket.bind(bindAddress)
//
//                Log.d("TCP", "Server started on $localIp:$UNICAST_PORT (interface: p2p0)")
//
//                while (true) {
//                    val clientSocket = serverSocket.accept()
//                    val clientIp = clientSocket.inetAddress.hostAddress
//                    Log.d("TCP", "Client connected: $clientIp")
//
//                    handleClient(clientSocket, onMessageReceived)
//                }
//
            ...
            CoroutineScope(Dispatchers.IO).launch {
                try {
                    ...
                    if (existing == null) {
                        ...
                        val senderSocket = Socket(senderIp, UNICAST_PORT)
                        val writer =
...
//                                            val goName = info.groupOwnerAddress?.hostName
    ...
    private fun broadcastUpdatedDeviceList(devices: List<DeviceInfo>) {
        ...
        devices.forEach { device ->
            try {
                val socket = Socket()
                socket.connect(InetSocketAddress(device.wfdIp, UNICAST_PORT), 2000)
                val writer = BufferedWriter(OutputStreamWriter(socket.getOutputStream()))
            ...
            for (device in allDevices) {
                if (!device.isGroupOwner && device.groupId == groupId) {
                    try {
                        ...
                        socket.connect(InetSocketAddress(device.wfdIp, UNICAST_PORT), 5000)
...
//                }
    ...
    fun sendMessageToServerAsWfd(hostAddress: String, message: String) {
        // infinix and samsung note 20 ultra 5G needs battery optimization for connection making
        // android version 14 does not allows to send this hello message to server device, need to see or
        // need to look, what makes those devices unable to send hello messages
        CoroutineScope(Dispatchers.IO).launch {
            try {
//                val p2pInterface = NetworkInterface.getByName("p2p0")
//                val localAddr = p2pInterface?.inetAddresses?.toList()
...
//                socket.bind(InetSocketAddress(localAddr, 0)) // bind to p2p0 interface
                ...
                socket.connect(InetSocketAddress(hostAddress, UNICAST_PORT), 5000)
    ...
    fun sendMessageToServerAsLc(hostAddress: String, message: String) {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                val packet = DatagramPacket(data, data.size, multicastGroup, UNICAST_PORT)
                ...
                Log.d(
                    "Multicast",
                    " Message sent to $multicastGroup:$UNICAST_PORT"
                )
...
//                    availableNetworks["p2p0"] = network
//                } else {
//                    availableNetworks["wlan0"] = network
//                }
//            }
//        }
//    }
...
//        CoroutineScope(Dispatchers.IO).launch {
//            try {
//                // Make sure our network list is up-to-date
//                updateAvailableNetworks()
//
//                // Pick correct interface based on IP
//                val isP2P = ipToSend.startsWith("192.168.49.")
//                val boundNetwork = if (isP2P) {
//                    availableNetworks["p2p0"]
//                } else {
//                    availableNetworks["wlan0"]
//                } ?: throw IllegalStateException("No suitable network found")
//
    ...
    private fun getNetworkForInterface(ifaceName: String): Network? {
        ...
        for (network in allNetworks) {
            ...
            if (capabilities != null && linkProperties != null) {
                if (linkProperties.interfaceName == ifaceName) {
                    return network
                }
            }
        }
